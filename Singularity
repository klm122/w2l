...
From: ubuntu:latest
%labels
MAINTAINER Mohammed-Amine KOULALI
SPECIES Dinosaur
...
# TorchMPI
MPI_CXX_COMPILER=/cache/usr/bin/mpicxx MPI_CXX_COMPILE_FLAGS="-O3" /cache/usr/luarocks make rocks/torchmpi-scm-1.rockspec

# wav2letter packages
git clone https://github.com/facebookresearch/wav2letter.git
cd wav2letter
cd gtn && /cache/usr/luarocks make rocks/gtn-scm-1.rockspec && cd ..
cd speech && /cache/usr/luarocks make rocks/speech-scm-1.rockspec && cd ..
cd torchnet-optim && /cache/usr/luarocks make rocks/torchnet-optim-scm-1.rockspec && cd ..
cd wav2letter && /cache/usr/luarocks make rocks/wav2letter-scm-1.rockspec && cd ..
